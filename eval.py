...
parser.add_argument('--model_file', default='neural-pcfg.pt')
...
def main(args):
    ...
    with torch.no_grad():
        for tree in open(args.data_file, "r"):
            ...
            sents = sents.cuda()
            nll, kl, binary_matrix, argmax_spans = model(sents,
                                                         argmax=True,
                                                         use_mean=(args.use_mean == 1))
    ...
    print('ReconPPL: %.2f, KL: %.4f, PPL Upper Bound from ELBO: %.2f' % (recon_ppl, kl, ppl_elbo))
