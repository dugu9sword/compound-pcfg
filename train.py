...
parser.add_argument('--save_path', default='neural-pcfg.pt', help='where to save the model')

# Model options
# Generative model parameters
parser.add_argument('--z_dim', default=0, type=int, help='latent dimension')
...
parser.add_argument('--print_every', type=int, default=100, help='print stats after N batches')
    ...
    while epoch < args.num_epochs:
        ...
        for i in np.random.permutation(len(train_data)):
            ...
            if b % args.print_every == 0:
                ...
                print(log_str % (epoch, b, len(train_data), param_norm, gparam_norm, args.lr,
                                 np.exp(train_nll / num_words), train_kl / num_sents,
                                 np.exp((train_nll + train_kl) / num_words), best_val_ppl,
                                 best_val_f1, all_f1[0], num_sents / (time.time() - start_time)))
                # print an example parse
                tree = get_tree_from_binary_matrix(binary_matrix[0], length)
                action = get_actions(tree)
                sent_str = [
                    train_data.idx2word[word_idx] for word_idx in list(sents[0].cpu().numpy())
                ]
...
def eval(data, model):
    ...
    print('ReconPPL: %.2f, KL: %.4f, PPL (Upper Bound): %.2f' % (recon_ppl, kl, ppl_elbo))
    print('Corpus F1: %.2f, Sentence F1: %.2f' % (corpus_f1 * 100, sent_f1 * 100))
    model.train()
