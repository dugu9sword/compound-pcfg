...
def get_tags_tokens_lowercase(line):
    ...
    for i in range(len(line_strip)):
        if i == 0:
            assert line_strip[i] == '('
        if line_strip[i] == '(' and not (is_next_open_bracket(
                line_strip, i)):  # fulfilling this condition means this is a terminal symbol
...
def get_data(args):
    ...
    print("Vocab size: Original = {}, Pruned = {}".format(len(indexer.vocab), len(indexer.d)))
    print(train_seqlength, valid_seqlength, test_seqlength)
    max_sent_l = 0
    max_sent_l = convert(args.testfile, args.lowercase, args.replace_num, args.batchsize,
                         test_seqlength, args.minseqlength, args.outputfile + "-test.pkl",
                         num_sents_test, max_sent_l, args.shuffle, args.include_boundary, 1)
    max_sent_l = convert(args.valfile, args.lowercase, args.replace_num, args.batchsize,
                         valid_seqlength, args.minseqlength, args.outputfile + "-val.pkl",
                         num_sents_valid, max_sent_l, args.shuffle, args.include_boundary, 1)
    max_sent_l = convert(args.trainfile, args.lowercase, args.replace_num, args.batchsize,
                         args.seqlength, args.minseqlength, args.outputfile + "-train.pkl",
                         num_sents_train, max_sent_l, args.shuffle, args.include_boundary, 1)
...
def main(arguments):
    ...
    parser.add_argument('--vocabminfreq',
                        help="Minimum frequency for vocab. Use this instead of "
                        "vocabsize if > 0",
                        type=int,
                        default=-1)
    ...
    parser.add_argument('--seqlength',
                        help="Maximum sequence length. Sequences longer "
                        "than this are dropped.",
                        type=int,
                        default=150)
    parser.add_argument('--minseqlength',
                        help="Minimum sequence length. Sequences shorter "
                        "than this are dropped.",
                        type=int,
                        default=0)
    ...
    parser.add_argument('--shuffle',
                        help="If = 1, shuffle sentences before sorting (based on  "
                        "source length).",
                        type=int,
                        default=0)
